<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Sistema de Teste da Vercan | Login</title>
  <link rel="icon" href="{{env('APP_URL')}}storage/images/favicon.png">

  <!-- Google Font: Source Sans Pro -->
  <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700&display=fallback">
  <!-- Font Awesome -->
  <link rel="stylesheet" href="storage/plugins/fontawesome-free/css/all.min.css">
  <!-- icheck bootstrap -->
  <link rel="stylesheet" href="storage/plugins/icheck-bootstrap/icheck-bootstrap.min.css">
  <!-- Theme style -->
  <link rel="stylesheet" href="storage/css/adminlte.min.css">
</head>
<body class="hold-transition login-page">
<div class="login-box">
  <div class="login-logo">
    <a href="index2.html"><b>Sistema de Teste da Vercan</b></a>
  </div>
  <!-- /.login-logo -->
  <div class="card">
    <div class="messageBoxSuccess d-none"></div>
    <div class="messageBoxDanger d-none"></div>
    <div class="card-body login-card-body">
      <p class="login-box-msg">Logue-se no sistema abaixo:</p>

      <form action="" method="post" id="formLogin">
        <div class="input-group mb-3">
          <input type="email" class="form-control" placeholder="Email" name="emailLogin" id="emailLogin" required>
          <div class="input-group-append">
            <div class="input-group-text">
              <span class="fas fa-envelope"></span>
            </div>
          </div>
        </div>
        <div class="input-group mb-3">
          <input type="password" class="form-control" placeholder="Password" name="senhaLogin" id="senhaLogin" required>
          <div class="input-group-append">
            <div class="input-group-text">
              <span class="fas fa-lock"></span>
            </div>
          </div>
        </div>
        <div class="row">
          <!-- /.col -->
          <div class="col-12">
            <input type='hidden' name="urlVolta" id="urlVolta" value="{{$_REQUEST['pagina']}}">
            <button type="submit" class="btn btn-primary btn-block">Login</button>
          </div>
          <!-- /.col -->
        </div>
      </form>

      <p class="mb-1">
        <a href="esqueci-minha-senha">Esqueci minha senha</a>
      </p>
    </div>
    <!-- /.login-card-body -->
  </div>
</div>
<!-- /.login-box -->

<!-- jQuery -->
<script src="storage/plugins/jquery/jquery.min.js"></script>
<!-- Bootstrap 4 -->
<script src="storage/plugins/bootstrap/js/bootstrap.bundle.min.js"></script>
<!-- AdminLTE App -->
<script src="storage/js/adminlte.min.js"></script>
<script src="storage/js/scripts.js"></script>
</body>
</html>
